...
Daily commit monitor and AI analysis for [triton](https://github.com/triton-lang/triton) and [triton-ascend](https://github.com/triton-lang/triton-ascend). Provides a knowledge base for AI agents to support triton-ascend code upgrades and main2main adaptation.
...
- **Patch Catalog Extraction** — Deterministic regex-based parsing of triton-ascend's `patch/__init__.py`, producing structured JSON without LLM costs
...
- **Adaptation Status Tracking** — Automatically tracks which triton upstream commits need adaptation, with only two states: `pending` (needs work) and `adapted` (covered by baseline). Regenerated on every fetch+analyze cycle.
...
- **MCP Server** — Stdio-based MCP server with 29 tools for AI agents to query the knowledge base. Supports progressive loading — agents can start with a lightweight overview and drill into specific modules as needed
...
│   │   ├── index.json
│   │   └── commits-index.json
│   └── triton-ascend/                  # triton-ascend project data
│       ├── meta.json
│       ├── commits/
│       ├── analysis/
...
├── src/
│   ├── __init__.py
│   ├── mcp_server_app.py             # MCP Server (stdio-based, 29 tools)
│   ├── data/
│   │   ├── __init__.py
...
The `init` subcommand also accepts `--data-dir` (default `data`) and `--local-repo` (triton source path, used as a git-log fallback when commits are missing the diff needed to classify).
...
# Only triton, skip deep analysis
...
Available flags: `--repo`, `--date`, `--triton-path`, `--ascend-path`, `--data-dir`, `--force`, `--skip-fetch`, `--skip-analyze`, `--skip-deep-analyze`, `--skip-build-index`, `--skip-track-adaptation`, `--skip-clean`, `--skip-pull`.
...
- **OpenCode** (native MCP): `opencode mcp add triton-report -- python -m src.mcp_server_app ...` or configure `mcp` in `opencode.json` / `opencode.jsonc`
...
- **`daily-commit.yml`** — Runs daily at 02:00 CST (or via manual `workflow_dispatch` with an optional date input): fetch → Phase 1 DeepSeek analysis → Phase 2 opencode analysis → build index → track adaptation → clean stale data → deploy. Runs for both triton and triton-ascend, checking out their sources into `repos/`.
- **`refresh-context.yml`** — Run on-demand (`workflow_dispatch`): checkout triton/triton-ascend source → generate architecture via opencode → cross-reference → rebuild index → track adaptation. (This replaced the old scheduled `weekly-context.yml`.)
...
| `data/triton/` | triton project data |
| `data/triton-ascend/` | triton-ascend project data |
| `data/triton-ascend/adaptation-status.json` | Adaptation progress tracking |
| `data/triton-ascend/lessons/` | Adaptation lessons recorded from main2main runs |

## Architecture Knowledge

The architecture knowledge is stored in `data/{repo}/context/architecture.json`, (re)generated on demand via the `refresh-context.yml` workflow or locally with `generate_context.py`, in both cases using the opencode agent to read actual source code. It covers **12 dimensions** across two files (~70KB for triton, ~90KB for triton-ascend).
...
| `modules` | ~6.5KB | Core module list (name, path, key_classes, description) — 23 modules for triton, 25 for triton-ascend | opencode (on-demand) |
...
| `cross_project_relationship` | ~13KB | Cross-project mapping — `patch_impact_map` (24 triton patched modules → ascend patch files), `impact_judgment_rules` (definitely/potentially/never_affected_paths), `triton_to_ascend_map` | opencode (on-demand, cross-reference) |
| `knowledge_base` | ~3KB (triton) / ~21KB (triton-ascend) | Knowledge base — see below for sub-fields | Mixed |
...
|-----------|------|-------------|---------------|
| `patch_catalog` | ~17.6KB (ascend) | All triton-ascend patches organized by category: `platform_patches` (21 patches), `worker_patches` (24 patches), `v2_worker_patches` (9 patches). Each patch entry includes `targets` (modified classes/functions), `why`, `how`, `related_pr`, `future_plan`. | `_extract_patches.py` — deterministic regex parsing of `patch/__init__.py`, no LLM cost |
| `development_workflows` | ~2.1KB | Fixed workflow templates for common development tasks: adding a new model, adding a config item, adding a platform backend (triton); adding platform patch, worker patch, new model, env variable, attention backend (triton-ascend). | Hardcoded in `generate_context.py` |
| `testing_guide` | ~0.6KB | Test commands and environment setup for both triton and triton-ascend: unit test commands, e2e test commands, lint commands, environment setup instructions. | Hardcoded in `generate_context.py` |

### cross_project_relationship sub-fields

| Sub-field | Description |
|-----------|-------------|
| `patch_impact_map` | Maps triton patched modules (e.g., `triton/v1/engine/core.py`) to corresponding triton-ascend patch files (e.g., `patch_balance_schedule.py`). 24 entries. |
...
| `triton_to_ascend_map` | Maps triton modules/classes to their ascend counterparts. |
| `ascend_only_components` | Components that exist only in triton-ascend (no upstream equivalent). |
...
The MCP server (`src/mcp_server_app.py`) provides **29 tools** across four categories plus an **experience/knowledge** category:
...
- `get_adaptation_baseline` — Return current triton baseline verified by triton-ascend
- `advance_baseline` — Advance the triton baseline (auto-marks covered commits as adapted)
...
**Change Analysis (6):**
- `get_ascend_impact_summary` — Summary of commits that affect triton-ascend
- `get_commit_diff` — Get full diff for a commit (local data first, GitHub API fallback)
- `get_commit_impact_batch` — Batch query ascend-impact analysis for a list of triton commits (structured JSON, used for deterministic routing)
...
- `get_cross_project_mapping` — Return triton ↔ triton-ascend cross-project mapping
- `get_patch_catalog` — Return triton-ascend patch catalog (optionally filtered by platform/worker category)
