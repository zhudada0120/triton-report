...
[triton](https://github.com/triton-lang/triton) 和 [triton-ascend](https://github.com/triton-lang/triton-ascend) 的每日 commit 监控与 AI 分析系统。提供知识库供 AI Agent（OpenCode 等）使用，支持 triton-ascend 上游代码人工 cherry-pick 回合与适配跟踪。
...
- **Patch 清单提取** — 通过 `_extract_patches.py` 对 triton-ascend 的 `third_party/ascend/patch/*.patch` 进行确定性 diffstat 解析（每个 patch 覆盖的文件、增删行数），生成结构化 JSON，无需 LLM 调用
- **源码上下文缓存** — 深分析前通过 AST 从源文件提取公开接口，Phase 2 无需额外 LLM 调用即可了解文件结构
- **检索索引** — 关键词/标签/模块索引，支持跨日期快速搜索
- **适配状态跟踪** — 自动跟踪哪些 triton 上游 commit 需要适配，仅维护两种状态：`pending`（待适配）和 `adapted`（已回合）。triton-ascend 无 baseline 文件，采用 git 历史扫描（上游 SHA 原样存在于 ascend 历史 / cherry-pick 标记）自动判定 adapted，无标记的人工回合用 `mark` 命令手动标记。每次 fetch+analyze 后自动重新检测。
- **适配经验库** — 从上游代码回合（人工 cherry-pick）中自动沉淀的实战适配知识（重复出现的冲突修复失败、新适配模式），AI Agent 可查询并按经验一步修复
- **MCP Server** — 29 个工具的 stdio 模式 MCP Server，供 AI Agent 查询知识库。支持渐进式加载——Agent 先获取轻量概览，再按需深入具体模块
...
├── src/
│   ├── __init__.py
│   ├── mcp_server_app.py             # MCP Server（stdio 模式，29 个工具）
│   ├── data/
│   │   ├── __init__.py
│   │   ├── _opencode_client.py       # opencode CLI 封装
│   │   ├── _extract_patches.py       # Patch 清单提取器（diffstat 确定性解析）
...
python src/data/fetch_commits.py --repo triton-lang/triton --local-repo ~/code/triton --date 2026-07-30
python src/data/fetch_commits.py --repo triton-lang/triton-ascend --local-repo ~/code/triton-ascend --date 2026-07-30

# 覆盖现有数据强制刷新
python src/data/fetch_commits.py --repo triton-lang/triton --local-repo ~/code/triton --date 2026-07-30 --force
```
...
```bash
# 为 triton 生成（使用 opencode Agent 读取源码）
python src/data/generate_context.py --repo triton-lang/triton --force

# 为 triton-ascend 生成
python src/data/generate_context.py --repo triton-lang/triton-ascend --force

# 交叉分析 triton 和 triton-ascend
...
python src/data/analyze_commits.py --repo triton-lang/triton

# 分析指定日期
python src/data/analyze_commits.py --repo triton-lang/triton --date 2024-01-15 --force

# 仅分析最新日期
python src/data/analyze_commits.py --repo triton-lang/triton --latest

# 显式 catch-up 模式（与默认行为相同）
python src/data/analyze_commits.py --repo triton-lang/triton --catch-up
```
...
```bash
python src/data/build_index.py --ascend-repo-path ~/code/triton-ascend
```

### 6. 跟踪适配状态

```bash
# 初始化跟踪（扫描分析结果，用 git 历史扫描自动判定 adapted）
...
# 每日刷新检测：将新检测到的 pending 提升为 adapted（保留手动标记）
python src/data/track_adaptation.py detect \
  --ascend-repo-path ~/code/triton-ascend

# 手动标记无标记的人工 cherry-pick 回合
python src/data/track_adaptation.py mark \
  --sha <sha12> --status adapted --note "ported in PR #1234"
...
`init` 子命令还支持 `--data-dir`（默认 `data`）。适配检测方式：上游 SHA 原样存在于 triton-ascend git 历史，或 ascend commit message 含 `(cherry picked from commit <sha>)` 标记。
...
# 只处理 triton，跳过深度分析
LLM_API_KEY=sk-xxx ./daily_refresh.sh --repo triton --skip-deep-analyze

# 查看全部选项
./daily_refresh.sh --help
```

可用参数：`--repo`、`--date`、`--triton-path`、`--ascend-path`、`--data-dir`、`--force`、`--skip-fetch`、`--skip-analyze`、`--skip-deep-analyze`、`--skip-build-index`、`--skip-track-adaptation`、`--skip-clean`、`--skip-pull`。
...
| `TRITON_REPO_PATH` | `--triton-path` | `$REPOS_DIR/triton` |
| `ASCEND_REPO_PATH` | `--ascend-path` | `$REPOS_DIR/triton-ascend` |
| `DATE` | `--date` | 昨天（北京时间） |
| `FORCE` | `--force` | `false` |
| `REPO` | `--repo` | （全部仓库） |
...
- **OpenCode**（原生 MCP）：`opencode mcp add triton-report -- python -m src.mcp_server_app ...` 或在 `opencode.json` / `opencode.jsonc` 中配置 `mcp`
...
- **`daily-commit.yml`** — 每天 02:00 CST 运行（或手动 `workflow_dispatch` 指定日期）：抓取 → Phase 1 DeepSeek 分析 → Phase 2 opencode 分析 → 构建索引 → 跟踪适配 → 清理过期数据 → 部署。同时处理 triton 和 triton-ascend，将源码检出到 `repos/`。
- **`refresh-context.yml`** — 按需（`workflow_dispatch`）：检出 triton/triton-ascend 源码 → 通过 opencode 生成架构 → 交叉分析 → 重建索引 → 跟踪适配。（取代了原定时 `weekly-context.yml`。）
...
| `data/triton/` | triton 项目数据 |
| `data/triton-ascend/` | triton-ascend 项目数据 |
| `data/triton-ascend/adaptation-status.json` | 适配进度跟踪 |
| `data/triton-ascend/lessons/` | 上游回合（人工 cherry-pick）中沉淀的适配经验 |

## 架构知识库

架构知识存储在 `data/{repo}/context/architecture.json` 中，按需（`refresh-context.yml` 工作流或本地 `generate_context.py`）由 opencode Agent 通过读取实际源码生成。覆盖 **12 个维度**，分别描述 triton（上游编译器）与 triton-ascend（Ascend 后端实现）的架构。
...
| `modules` | 按项目规模 | 核心模块列表（名称、路径、关键类、描述） | opencode（按需） |
| `key_abstractions` | 按项目规模 | 关键抽象/类，含继承关系（`inherits_from`）、关键方法签名、ascend 实现类名 | opencode（按需） |
| `implementation_principles` | 按项目规模 | 实现原理——核心工作流（编译流水线、后端注册机制、Runtime 启动流程等） | opencode（按需） |
| `module_dependencies` | 按项目规模 | 模块间依赖关系文字描述 | opencode（按需） |
| `hardware_abstraction` | 按项目规模 | 硬件适配层——平台无关接口 vs 平台特有实现 | opencode（按需） |
| `interface_surface` | 按项目规模 | 接口面——`inheritable_interfaces`（ascend 继承/复写的接口）+ `not_used_by_ascend`（可跳过的平台特化路径） | opencode（按需） |
| `test_structure` | 按项目规模 | 测试结构概览 | opencode（按需） |
| `cross_project_relationship` | 按项目规模 | 跨项目映射——`patch_impact_map`（triton 被 patch 模块→ascend patch 文件）、`impact_judgment_rules`（必然/可能/绝不影响路径）、`triton_to_ascend_map` | opencode（按需，交叉分析） |
| `knowledge_base` | ~3KB (triton) / ~21KB (triton-ascend) | 知识库——见下方子字段说明 | 混合 |
...
| 子字段 | 大小 | 说明 | 更新方式 |
|--------|------|------|---------|
| `patch_catalog` | 按 patch 文件数量 | triton-ascend `third_party/ascend/patch/*.patch` 清单：每个 patch 文件的名称、大小、覆盖的目标文件（per-file diffstat：增删行数） | `_extract_patches.py` 对 `.patch` 文件进行确定性 diffstat 解析，无 LLM 成本 |
| `development_workflows` | ~2KB | 固定开发工作流模板：添加算子/语言特性、编译器 pass、新硬件后端（triton）；Ascend 后端开发、上游代码回合、算子支持、patch 维护（triton-ascend） | 硬编码在 `generate_context.py` 中 |
| `testing_guide` | ~0.5KB | 测试命令和环境设置：lit/pytest 测试、lint 命令、环境准备步骤 | 硬编码在 `generate_context.py` 中 |

### cross_project_relationship 子字段

| 子字段 | 说明 |
|--------|------|
| `patch_impact_map` | triton 被 patch 模块（如 `python/triton/backends/compiler.py`）到对应 triton-ascend patch 文件（如 `triton-ascend-3.6.0.patch`）的映射 |
| `impact_judgment_rules` | 三类路径匹配规则：`definitely_affected_paths`（修改必然影响 ascend）、`never_affected_paths`（纯 NVIDIA/AMD 后端等平台特化代码）、`potentially_affected_paths`（需要人工判断） |
| `triton_to_ascend_map` | triton 模块/类到 ascend 对应实现的映射 |
| `ascend_only_components` | 仅 triton-ascend 特有的组件（上游无对应） |

### 动态维护机制

- **`interface_surface.not_used_by_ascend`** — 每次架构刷新时由 opencode 自动更新，始终反映当前代码库状态：哪些路径是纯 NVIDIA/AMD 平台特化代码、ascend 绝不会触及
- **`patch_catalog`** — 每次 `third_party/ascend/patch/*.patch` 变更时由 `_extract_patches.py` 自动更新，无需 LLM 调用
...
```
Agent 首次调用：    get_architecture_overview("triton-ascend")        → ~1KB 概览
Agent 二次调用：    get_module_info("triton-ascend", "attention")     → ~1KB 模块详情
Agent 三次调用：    get_interface_surface("triton")                   → ~8KB 接口面
按需调用：          get_development_workflows("triton-ascend")        → ~2KB 工作流
                    get_implementation_principles("triton")           → ~5KB 实现原理
                    get_patch_catalog("triton-ascend-3.6.0")       → 按 patch 文件名前缀筛选
```
...
MCP Server（`src/mcp_server_app.py`）提供 **29 个工具**，分为四类：
...
**适配管理（6 个）：**
- `get_adaptation_baseline` — 返回适配跟踪状态（history-scan 模式、跟踪起始、检测时间、pending/adapted 统计）
- `detect_adaptation` — 在 triton-ascend git 历史中检测已适配的上游 commit（pending → adapted）
- `update_adaptation_status` — 手动标记某个 commit 的适配状态（无标记的人工 cherry-pick 回合用）
...
- `get_commit_impact_batch` — 批量查询一批 triton commit 的 ascend 影响分析（结构化 JSON，用于 PI 决策确定性路由）
...
- `get_patch_catalog` — 返回 triton-ascend patch 文件清单（diffstat，可选按文件名前缀筛选）
